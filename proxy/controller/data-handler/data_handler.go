...
import (
	...
	"github.com/tp-distribuidos-2c2025/proxy/controller/data-handler/routing"
...
func NewDataHandler(config *middleware.ConnectionConfig) *DataHandler {
	return &DataHandler{
		config:  config,
		router:  routing.NewFileTypeRouter(),
	}
}
...
func (dh *DataHandler) ProcessBatchMessage(data []byte) error {
	...
	fileType := dh.router.ClassifyFile(batchMsg.FileID)

	switch fileType {
	case routing.FileTypeReference:
		// Send reference data (stores, menu_items, users) to join-data-handler for routing to join workers
		queryType := dh.router.GetQueryType(batchMsg.FileID)

		chunkObj := dh.createChunk(batchMsg, queryType)
	...
	case routing.FileTypeTransaction:
		// Send all transaction data to year-filter (non-reference data)
		queryTypes := dh.router.GetQueryTypesForTransactionFile(batchMsg.FileID)
