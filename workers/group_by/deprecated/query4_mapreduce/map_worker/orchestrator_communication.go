...
func NewOrchestratorCommunicator(mapWorkerID string, config *middleware.ConnectionConfig) *OrchestratorCommunicator {
	...
	chunkNotificationProducer := workerqueue.NewMessageMiddlewareQueue(queues.Query4OrchestratorChunksQueue, config)
	...
	terminationConsumer := exchange.NewExchangeConsumer(queues.Query4MapTerminationExchange, []string{""}, config)
	...
	exchangeDeclarer := exchange.NewMessageMiddlewareExchange(queues.Query4MapTerminationExchange, []string{}, config)
	if exchangeDeclarer == nil {
		chunkNotificationProducer.Close()
		terminationConsumer.Close()
		log.Fatalf("Failed to create exchange declarer for exchange: %s", queues.Query4MapTerminationExchange)
	}
	if err := exchangeDeclarer.DeclareExchange("fanout", false, false, false, false); err != 0 {
		chunkNotificationProducer.Close()
		terminationConsumer.Close()
		exchangeDeclarer.Close()
		log.Fatalf("Failed to declare termination exchange %s: %v", queues.Query4MapTerminationExchange, err)
	}
