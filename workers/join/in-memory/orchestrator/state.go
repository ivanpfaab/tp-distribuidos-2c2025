...
func (sm *StateManager) RebuildState() error {
	...
	for _, file := range files {
		...
		if err != nil {
			testing.LogWarn("In-Memory Join Orchestrator", "Failed to check incomplete row in %s: %v", filePath, err)
			continue
		}
		if hasIncomplete {
			if err := sm.csvHandler.RemoveIncompleteLastRow(filePath); err != nil {
				testing.LogWarn("In-Memory Join Orchestrator", "Failed to remove incomplete row in %s: %v", filePath, err)
				continue
			}
		...
		minRequiredFields := len(expectedHeader) - 1 // Support old format without map_worker_id

		// Read header first to validate format
		file, err := os.Open(filePath)
		if err != nil {
			if os.IsNotExist(err) {
				continue
			}
			testing.LogWarn("In-Memory Join Orchestrator", "Failed to open file %s: %v", filePath, err)
			continue
		}
		...
		file.Close()
		if err != nil {
			testing.LogWarn("In-Memory Join Orchestrator", "Failed to read header from %s: %v", filePath, err)
			continue
		}
		if len(header) < minRequiredFields {
			testing.LogWarn("In-Memory Join Orchestrator", "Invalid header in %s: expected at least %d columns, got %d", filePath, minRequiredFields, len(header))
			continue
		}

		// Process rows line-by-line
		err = sm.csvHandler.ReadFileStreaming(filePath, func(row []string) error {
		...
		if sm.completionTracker.IsClientCompleted(clientID) {
			...
			rebuiltCount++
			testing.LogInfo("In-Memory Join Orchestrator", "Rebuilt state for client %s (%d notifications)", clientID, notificationCount)
		}
	}
...
func (sm *StateManager) AppendNotification(notification *signals.ChunkNotification) error {
	...
	columns := []string{"notification_id", "client_id", "file_id", "table_id", "chunk_number", "is_last_chunk", "is_last_from_table", "map_worker_id"}
